...
namespace PromiCRM.Controllers
{
    ...
    [ApiController]
    public class NonStandardWorkController : Controller
    {
        ...
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> DeletenoNStandardWork(int id)
        {
            var nonStandardWork = _unitOfWork.NonStandardWorks.Get(c => c.Id == id);
            if (nonStandardWork == null)
            {
                _logger.LogError($"Invalid DELETE attempt in {nameof(DeletenoNStandardWork)}");
                return BadRequest("Submited invalid data");
            }
